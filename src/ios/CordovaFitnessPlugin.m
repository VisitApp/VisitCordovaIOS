/********* CordovaFitnessPlugin.m Cordova Plugin Implementation *******/
...
@interface CordovaFitnessPlugin : CDVPlugin <WKScriptMessageHandler> {
...
@implementation CordovaFitnessPlugin
...
-(void) canAccessHealthKit: (void(^)(BOOL))callback {
    ...
    [[CordovaFitnessPlugin sharedManager] saveObject:sample withCompletion:^(BOOL success, NSError *error) {
            if (!success) {
                NSLog(@"An error occured saving the step count sample %@. The error was: %@.", sample, error);
                callback(NO);
            }else{
                [[CordovaFitnessPlugin sharedManager] deleteObject:sample withCompletion:^(BOOL success, NSError * _Nullable error) {
                    if(!success){
                        callback(NO);
                    }else{
                        callback(YES);
...
{
    NSError *error;
    HKBiologicalSexObject *gen=[[CordovaFitnessPlugin sharedManager] biologicalSexWithError:&error];
